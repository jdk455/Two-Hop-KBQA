...
def RE_Stage1(question_body,schema):
    ...
    print("RE_stage1_response",res)
    
    res=res[res.find("["):res.find("]")+1]
    res=res[1:len(res)-1].split(",")
    print("processed_RE_stage1_response",res)
    return res
    # print(res)
...
def RE_Stage2(ques,avai_event,schema):
    print("--------------------stage2")
    res_dict=dict()
    for i in avai_event:
        i=i.strip()
        res_dict[i]=(RE_Stage2_onestep(ques,i,schema))
    return res_dict

def RE_Stage2_onestep(question_body,one_event,schema):
    # global question_body,entity_extraction,property_extraction
    stage2_question_head='请在下面一段话提取有关于"'+one_event+'"的三元组及对应名称，并以json格式返回。其中关系列表有['+",".join(schema[one_event])+']，例如当关系列表为[推荐药物]时，一种可能的回答是{"has_symptom":["fever","heating"]}，若句子中没有"has_symptom"的关系，就回答{"has_symptom":"none"}。问题："'
    stage2_question=stage2_question_head+question_body+'"'
    print("RE_Stage2_onestep",stage2_question)
    response=openai.ChatCompletion.create(
    model="gpt-3.5-turbo",
    ...
    print("Stage2回答：",one_event,"所获得的三元组为：",res)
...
def RE_build_graph(entity_extraction):
    print("RE_build_graph",entity_extraction)
    match_cypher="MATCH "
    return_cypher="\nRETURN "
    ...
    with driver.session() as session:
        for one_extraction in entity_extraction:
            ...
            for i,j in one_extraction_dict.items():
                    if len(j)==2:
                        match_cypher=match_cypher+f"({j[0]}:CommonNode{{name:'{j[0]}'}})-[{j[0]}_{j[1]}:{i}]-({j[1]}:CommonNode{{name:'{j[1]}'}}),"
                        return_cypher=return_cypher+j[0]+","+j[1]+","+j[0]+"_"+j[1]+","
                        exec_cypher=f'''MERGE ({j[0]}:CommonNode {{name: '{j[0]}'}})
                                    MERGE ({j[1]}:CommonNode {{name: '{j[1]}'}})
                                    CREATE ({j[0]})-[:{i} {{name: '{i}'}}]->({j[1]})'''
                        cypher_exec(driver.session(database="neo4j"),exec_cypher)
    all_cypher=match_cypher[:len(match_cypher)-1]+return_cypher[:len(return_cypher)-1]
    print("all_cypher",all_cypher)
    return all_cypher
...
def test(now_event):
    ...
    gpt不听话=0
    try:
        avai_events=RE_Stage1(now_event,event_type,schema)
        print("Stage1回答：",avai_events)
        avai_args=RE_Stage2(now_event,avai_events,schema)
    except:
        gpt不听话+=1
        print("gpt不听话")
        avai_args="fuck"
